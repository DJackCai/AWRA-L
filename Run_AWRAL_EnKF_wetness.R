 ##### Assimilation of SMAP soil moisture data into the AWRA-L model, using wetness ###### 
...
Run_AWRAL_EnKF_wetness_new <- function(catname, Ne, start_date, end_date, store_length, 
                                   SMAP_Err = NULL, 
                                   Rain_Err = 0.6 , Temp_Err = 2.5 , Rad_Err = 50, IF = 1.1) {
  
  # catname : catchment name (file name)
  ...
  ## PARAM set & N_GRID ####
  STATIC_PAR <- as.matrix(read.csv(paste0(catname,"static.csv")))
  ...
  PARAMS <- PARAMS_set(rbind(rep(1,N_GRID),rep(1,N_GRID)),STATIC_PAR =STATIC_PAR )
  ...
  OL_run =  awra_run_func(N_grid = N_GRID, ALL_FORCING = all_forcing,
                          STATES = STATES, PARAMS = PARAMS)
  
  ##### Convert SMAP observation to wetness from 2015-2018 #####
  
  SMAP_data  = as.matrix(read.csv(paste0(catname, "SMAP1519.csv")))[1:1461,paste0("grid",1:N_GRID)]
  S0mean_sim = OL_run$S0mean[-c(1:365),]
  ...
  SMAP_wetness_scaled  = mapply(function(x,y) { 
    ...
    as.data.frame(AWRA_wetness))
  
  
  if (is.null(SMAP_Err)) {
    SMAP_Err = apply(SMAP_wetness_scaled,2,Diff_error_estim) } else {
  ...
  QTOT_f = ET_f = FSAT_f = matrix(NA,Ne,N_GRID)
  ...
  FSAT_median =  FSAT_mean   = FSAT_75 = FSAT_25 =  FSAT_max = FSAT_min = matrix(NA,store_length,N_GRID)
  ...
  for (k in 1:NL) {
    ...
    if (k <=365) {  
      ...
      ### Inflate ensemble variance
      
      Xf_S01 = IF * (Xf_S01 - matrix(colMeans(Xf_S01, na.rm = T),Ne,N_GRID,byrow=T)) + 
        matrix(colMeans(Xf_S01),Ne,N_GRID,byrow=T)
      Xf_S02 = IF * (Xf_S02 - matrix(colMeans(Xf_S02, na.rm = T),Ne,N_GRID,byrow=T)) + 
        matrix(colMeans(Xf_S02),Ne,N_GRID,byrow=T)
      ...
      stopifnot(length(which(is.na(Xa_S01)==T)) == 0)
      stopifnot(length(which(is.na(Xa_S02)==T)) == 0)
      stopifnot(length(which(is.na(Xa_Ss1)==T)) == 0)
      stopifnot(length(which(is.na(Xa_Ss2)==T)) == 0)
      ...
      S02_ens10[k-365,] = Xa_S01[10,]  # ensemble member 10 for all grids
      S02_ens20[k-365,] = Xa_S01[20,]  # ensemble member 20 for all grids
      S02_ens30[k-365,] = Xa_S01[30,]
      ...
      FSAT_min[k-365,] = apply(FSAT_f,2,function(x) {min(x,na.rm=T)} )
      FSAT_max[k-365,] = apply(FSAT_f,2,function(x) {max(x,na.rm=T)})
     ...
     # w0_a  =  matrix(PARAMS$Fhru[1,] / PARAMS$S0FC[1,],Ne,N_GRID,byrow=T) * Xa_S01 +
      ...
      if (k%%200 ==0 ) {print(paste0("Ensemble run for time ", k, " is finished.")) } 
    }
  }
  
  return(list(S0_min_f = S0_min_f, S0_max_f = S0_max_f, S0_75_f = S0_75_f, S0_25_f = S0_25_f,
              ...
              FSAT_max = FSAT_max, FSAT_min = FSAT_min, 
